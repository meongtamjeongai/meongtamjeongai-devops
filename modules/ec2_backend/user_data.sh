...
# ⭐️ 중요: fastapi_docker_image_placeholder에는 ECR 리포지토리 URL(태그 제외)이 전달되어야 합니다.
...
# `describe-images` 명령의 성공 여부로 이미지 존재를 판단. 에러는 /dev/null로 보내고, 성공 시에만 0을 반환.
if sudo aws ecr describe-images --repository-name "${ECR_REPOSITORY_NAME}" --region "${AWS_REGION}" > /dev/null 2>&1; then
    ...
    # 실패: 이미지가 없음 (ImageNotFoundException 등)
